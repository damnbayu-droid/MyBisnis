...
import { MapPin, Star, CreditCard, Share2, MessageCircle, Store, ShoppingBag, Home } from 'lucide-react'
...
const STORE_DATA = {
    name: "Toko Demo Bayu",
    description: "Ini adalah Halaman Toko Demo untuk presentasi. Semua data di sini adalah dummy dan tidak terhubung ke database.",
    address: "Jl. Demo Presentation No. 1, Jakarta",
    rating: 5.0,
    reviews: 999,
    banner: "bg-gradient-to-r from-blue-900 to-slate-900",
    paymentMethods: ["QRIS", "Transfer Bank", "COD"],
    isVerified: true
}
...
export default function DemoStorePage() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 font-sans text-slate-100 pb-20">
            ...
            <header className="fixed top-[34px] w-full z-40 bg-slate-900/80 backdrop-blur-md border-b border-slate-800">
                <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <Link href="/proposal" className="font-bold text-lg bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">
                            MyBisnis Demo
                        </Link>
                    </div>
            ...
            <main className="max-w-7xl mx-auto px-6 mt-20 md:mt-12 space-y-8">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 space-y-4">
                        <div>
                            <h2 className="text-lg font-bold text-white mb-2">Tentang Toko (DEMO)</h2>
                ...
                <div>
                    <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                        <ShoppingBag className="w-5 h-5 text-amber-500" /> Daftar Produk (DEMO)
                    </h2>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {sampleProductsId.map((product, idx) => (
                            <ProductCard
                                ...
                                onAddToCart={(e) => {
                                    e.stopPropagation()
                                    toast.success("Produk ditambahkan ke keranjang (Demo)")
                                }}
                                onBuyNow={(e) => {
                                    e.stopPropagation()
                                    toast.success("Melanjutkan ke pembayaran... (Demo)")
                                }}
                                onShare={(e) => {
                                    e.stopPropagation()
                                    toast.success("Link produk disalin! (Demo)")
                                }}
                            />
                        ))}
                    </div>
